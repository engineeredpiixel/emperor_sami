...
export default function LocationHero({ slug, fallback }: { slug: string, fallback: TerritoryType }) {
  const { t } = useCMS();
  
  const heroImg = t(`territory.${slug}.heroImage`, fallback.heroImage);
  const targetLock = t(`territory.${slug}.heroTargetLock`, `Target Lock: ${fallback.name}`);
  
  const fallbackWords = fallback.heroHeadline.split(' ');
  const p1Fallback = fallbackWords.slice(0, fallbackWords.length - 2).join(' ');
  const p2Fallback = fallbackWords.slice(fallbackWords.length - 2).join(' ');
  
  const titleTop = t(`territory.${slug}.heroTitleTop`, p1Fallback);
  const titleBot = t(`territory.${slug}.heroTitleBot`, p2Fallback);
  
  const heroDesc = t(`territory.${slug}.heroDesc`, fallback.heroSubheadline);
    ...
    <section className="relative w-full h-[85vh] min-h-[600px] pt-20 flex items-center overflow-hidden border-b-8 border-b-white/5 bg-black">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 lg:px-12 flex flex-col pt-24 pb-12">
         ...
         <div className="flex items-center gap-4 mb-8">
             ...
             <span className="text-white text-[10px] sm:text-xs font-black tracking-[0.6em] uppercase">{targetLock}</span>
         ...
         <div className="flex flex-col gap-6 max-w-4xl">
            <h1 className="text-white text-5xl sm:text-7xl md:text-8xl lg:text-[6.5rem] font-black tracking-tighter uppercase leading-[0.9] drop-shadow-[0_10px_30px_rgba(0,0,0,1)]">
               {titleTop}{' '}
               <span className="text-[#D8A02A] italic pr-4">
                  {titleBot}
               </span>
            </h1>
