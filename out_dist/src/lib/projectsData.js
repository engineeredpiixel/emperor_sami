...
exports.hydrateProjectsWithCMS = exports.getSortedProjects = exports.masterProjects = void 0;
var servicesDataResidential_1 = require("./servicesDataResidential");
var servicesDataCommercial_1 = require("./servicesDataCommercial");
var LOCATIONS = [
...
exports.masterProjects = {};
// ── THE PROCEDURAL GENERATOR ENGINE ──
// This wildly powerful loop merges 40 Services x 3 Locations to yield 120 distinct items.
var globalCounter = 0;
function generateProjectsForService(serviceKey, serviceTitle, serviceImage, isResidential) {
    var templates = isResidential ? RESIDENTIAL_TEMPLATES : COMMERCIAL_TEMPLATES;
    LOCATIONS.slice(0, 3).forEach(function (loc, i) {
        ...
        var titlePrefixes = ["The", "", "".concat(loc.name, " Elite"), "Strategic", "Premium"];
        var prefix = titlePrefixes[i % titlePrefixes.length];
        var finalTitle = "".concat(prefix ? prefix + " " : "").concat(loc.name, " ").concat(serviceTitle).trim();
        // Create a deterministic unique slug
        var finalSlug = "".concat(loc.name.toLowerCase().replace(/ /g, '-'), "-").concat(serviceKey, "-exec-").concat(i);
        ...
        var locNameLower = loc.name.toLowerCase();
        var uniqueHeroImage = "/optimized_v2/prj_".concat(locNameLower, "_").concat(serviceKey, "_hero.webp");
        var uniqueGallery = [
            "/optimized_v2/prj_".concat(locNameLower, "_").concat(serviceKey, "_gal1.webp"),
            "/optimized_v2/prj_".concat(locNameLower, "_").concat(serviceKey, "_gal2.webp")
        ];
        ...
        var seed = (globalCounter * 0.013) % 0.05;
        var offsetLat = loc.baseLat + (i % 2 === 0 ? seed : -seed);
        var offsetLng = loc.baseLng + (i % 3 === 0 ? seed : -seed);
        exports.masterProjects[finalSlug] = {
            slug: finalSlug,
            title: finalTitle,
            category: serviceTitle, // Strictly matching the Dropdowns
            division: isResidential ? "Residential" : "Commercial",
            location: "".concat(loc.name, ", Greater Toronto Area"),
            lat: offsetLat,
            lng: offsetLng,
            ...
            testimonial: __assign(__assign({}, template.testimonial), { 
                // Append the city dynamically to make the testimonials feel incredibly localized
                role: "".concat(loc.name, " ").concat(template.testimonial.role) }),
...
Object.entries(servicesDataResidential_1.residentialServicesData).forEach(function (_a) {
...
Object.entries(servicesDataCommercial_1.commercialServicesData).forEach(function (_a) {
    var slug = _a[0], data = _a[1];
...
var getSortedProjects = function () {
    return Object.values(exports.masterProjects).sort(function (a, b) {
        var indexA = PRIORITY_FOCUS.indexOf(a.category);
        var indexB = PRIORITY_FOCUS.indexOf(b.category);
...
var hydrateProjectsWithCMS = function (staticProjects, globalContent) {
    var projectKeys = globalContent.filter(function (c) { return c.key.startsWith('project.'); });
    if (projectKeys.length === 0)
        return staticProjects;
    // Group overrides by slug
    var overrides = {};
    projectKeys.forEach(function (item) {
    ...
    var clonedProjects = staticProjects.map(function (p) {
        var over = overrides[p.slug];
        if (!over)
            return p;
        return __assign(__assign({}, p), { title: over.title || p.title, category: over.category || p.category, division: over.division || p.division, location: over.location || p.location, heroImage: over.heroImage || p.heroImage, metrics: {
    ...
    var staticSlugs = new Set(staticProjects.map(function (p) { return p.slug; }));
    var newSlugs = Object.keys(overrides).filter(function (s) { return !staticSlugs.has(s); });
    newSlugs.forEach(function (slug) {
        var over = overrides[slug];
        // Look up lat/lng based on location if possible, otherwise default Toronto
        var locData = LOCATIONS.find(function (l) { return l.name === over.location; });
        var lat = (locData === null || locData === void 0 ? void 0 : locData.baseLat) || 43.65107;
        var lng = (locData === null || locData === void 0 ? void 0 : locData.baseLng) || -79.347015;
        clonedProjects.push({
            slug: slug,
...
exports.hydrateProjectsWithCMS = hydrateProjectsWithCMS;
